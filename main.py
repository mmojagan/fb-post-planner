...
def write_template(self,template,template_values=None):
    header = JINJA_ENVIRONMENT.get_template('templates/navbar.html')
...
class MainHandler(webapp2.RequestHandler):
    ...
    def post(self):
        ...
        post.message = self.request.get("message" + Str1)
        ...
        post.put()
        self.response.write('<script>alert("Your Post has been Scheduled.");window.location.assign("/list/'+post.user_id+'")</script>')
...
class PostToFBHandler(webapp2.RequestHandler):
    def post(self):
        data = {
                    "method": "post",
                    "message": self.request.get("message")+ " : Team 18",
                    "access_token": self.request.get("access_token")
                };        
        post = Posts()
